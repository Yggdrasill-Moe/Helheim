...
void decompression(unit32 FileNum, char *dname)
{
	...
	unit32 i, hflag, filen, noffset, j;
	unit16 flag;
	uLong flen, ulen;
	_chdir(dname);
	for (i = 0; i < FileNum; i++)
	{
		...
		else
		{
			...
			if (hflag == 0x205)
			{
				...
				printf("%s flag:0x%X\n", Index[i].FileName, hflag);
				fseek(OpenFile, 8, SEEK_CUR);
				for (j = 0; j < filen; j++)
				{
					fread(&FIndex[j].FileNameOff, 4, 1, OpenFile);
					FIndex[j].FileNameOff = EndianChange(FIndex[j].FileNameOff) + 0x20;
				}
				for (j = 0; j < filen; j++)
				{
					fread(&FIndex[j].FileOffset, 4, 1, OpenFile);
					FIndex[j].FileOffset = EndianChange(FIndex[j].FileOffset) + 0x20;
				}
				for (j = 0; j < filen; j++)
				{
